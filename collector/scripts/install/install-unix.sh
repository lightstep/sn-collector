...
# out_file_path is the full path to the downloaded package (e.g. "/tmp/sn-collector_linux_amd64.deb")
...
Usage:
  $(fg_yellow '-v, --version')
      Defines the version of the ServiceNow OpenTelemetry Collector.
  ...
  $(fg_yellow '-l, --url')
      ...
      If not provided, this will default to ServiceNow OpenTelemetry Collector\'s GitHub releases.
      Example: '-l http://my.domain.org/sn-collector' will download from there.
  ...
  $(fg_yellow '-b, --base-url')
      ...
      Example: '-b http://my.domain.org/sn-collector/binaries' will be used as the base of the download URL.
...
set_download_urls()
{
  if [ -z "$url" ] ; then
    ...
    collector_download_url="$base_url/v$version/${PACKAGE_NAME}_${version}_linux_${os_arch}.${package_type}"
  else
...
install_package()
{
  banner "Installing ServiceNow OpenTelemetry Collector"
  increase_indent
  ...
  if [ ! -d "/opt/sn-collector" ] && [ "$package_type" = "deb" ]; then
    dpkg -s "sn-collector" > /dev/null 2>&1 && dpkg --purge "sn-collector" > /dev/null 2>&1
  fi
  ...
  if [ "$(systemctl is-enabled sn-collector)" = "enabled" ]; then
    ...
    systemctl restart sn-collector
    succeeded
  else
    info "Enabling service..."
    systemctl enable --now sn-collector > /dev/null 2>&1 || error_exit "$LINENO" "Failed to enable service"
    succeeded
  fi

  success "ServiceNow OpenTelemetry Collector installation complete!"
  decrease_indent
}
...
display_results()
{
    banner 'Information'
    increase_indent
    info "Collector Home:         $(fg_cyan "/opt/sn-collector")$(reset)"
    ...
    increase_indent
    info "$(fg_cyan "https://github.com/lightstep/sn-collector")$(reset)"
    decrease_indent
    info "If you have any other questions please open a case at support.servicenow.com$(reset)"
    increase_indent
    decrease_indent
    decrease_indent
...
uninstall_package()
{
  case "$package_type" in
    deb)
      dpkg -r "sn-collector" > /dev/null 2>&1
      ;;
    rpm)
      rpm -e "sn-collector" > /dev/null 2>&1
      ;;
    *)
      error "Unrecognized package type"
      return 1
...
{
  set_package_type
  banner "Uninstalling ServiceNow OpenTelemetry Collector"
  increase_indent
  ...
  systemctl stop sn-collector > /dev/null || error_exit "$LINENO" "Failed to stop service"
  succeeded

  info "Disabling service..."
  systemctl disable sn-collector > /dev/null 2>&1 || error_exit "$LINENO" "Failed to disable service"
  succeeded
