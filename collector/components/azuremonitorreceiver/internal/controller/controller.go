...
package controller // import "github.com/open-telemetry/opentelemetry-collector-contrib/receiver/azuremonitorreceiver/internal/controller"
...
// Patterns in this package are based on scraper/helperscrapercontroller.go from the
// OpenTelemetry Collector. We're justing using it to manage Logs and Traces scrapes
// right now, since Metrics is handled in the regular Scraper.
...
// collection interval. Since `scrapeMetricsAndReport` is nilladic, it's simple
// to specialize this for any other type of scraper.
...
// In the Collector, scrapeMetricsAndReport is the Scrape function for each of the configured
// Scrapers. Since the receiver needs this for Logs and Traces, the code can generailze at
// the level of nilladic functions closed over ops.
func (sc *controller) scrapeLogsAndReport() {
	ctx, done := withScrapeContext(sc.timeout)
	defer done()

	// TODO: start initalizing then scrape.
