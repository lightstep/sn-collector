...
package azure // "github.com/open-telemetry/opentelemetry-collector-contrib/pkg/translator/azure"
...
func (r ResourceLogsUnmarshaler) UnmarshalLogs(buf []byte) (plog.Logs, error) {
	...
	for _, resourceID := range resourceIds {
		...
		for i := 0; i < len(logs); i++ {
			...
			if err != nil {
				r.Logger.Warn("Unable to convert timestamp from log: " + log.Time, zap.String("timestamp", log.Time))
				r.Logger.Info("Time: " + log.Time, zap.String("record.Time", log.Time))
				r.Logger.Info("Timestamp: " + log.Timestamp, zap.String("record.Timestamp", log.Timestamp))
				continue
			}
